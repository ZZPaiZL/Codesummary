...
/*
 * jQuery Multimedia Gallery Slideshow with MousePan v1.86
 * http://codecanyon.net/item/jquery-image-gallery-slideshow-with-mousepan/310165
 */

(function($) {

	$.fn.panGallery = function(settings, ap_settings) {
	...
	//icons
	var ic_fullscreen_enter = '../static/imgs/fullscreen_enter.png';
	var ic_fullscreen_exit = '../static/imgs/fullscreen_exit.png';
    var ic_image_toggle_plus = '../static/imgs/image_toggle_plus.png';
	var ic_image_toggle_minus = '../static/imgs/image_toggle_minus.png';
	var ic_info_toggle = '../static/imgs/info_toggle.png';
	var ic_info_close = '../static/imgs/close.png';
	var ic_slideshow_play = '../static/imgs/slideshow_play.png';
	var ic_slideshow_pause = '../static/imgs/slideshow_pause.png';
	
	var ic_music = '../static/imgs/music.png';
	var ic_music_on = '../static/imgs/music_on.png';	
	
	
	var componentFixedSize = false;
	var dontAlignGalleryControls=settings.dontAlignGalleryControls;
	...
	var _currentInsert, _currentInsertRemove;
	
	var categoryTransitionOn=true;
	
	var windowResizeTimeoutID;
	var windowResizeTimeout = 500;//execute resize after time finish
	
	var address_intro = false;
	
	var scrollType = settings.scrollType;
	var scrollPane, scrollPaneApi, scroll_size=17, _isScrollable;
	...
	if(componentWrapper.find('.info_wrapper').length>0){
		...
		var infoExist=false;
		var info_toggle = componentWrapper.find('.info_toggle').css('cursor','pointer').bind('click', toggleInfo);
	...
	if(slideshowOn) slideshowToggleSrc.attr('src', "../static/imgs/slideshow_pause.png");
	...
	if(useVideo){
		...
		var videoPlayerHolder = $(settings.playerHolder), videoExistInSlide=false, videoType, videoLink;
		...
		function toggleVideoPlayer(dir, instant){
			...
			if(dir=='on'){
				...
				videoPlayerHolder.stop().animate({ 'opacity':0},  {duration: toggle_time, easing: 'easeOutSine', complete:function(){
					videoPlayerHolder.css('left',-10000+'px');
					videoTransitionOn=false;
	...
	if(useAudio){
		var music_toggle = componentWrapper.find('.music_toggle').css('cursor', 'pointer').bind('click', function(){
			if(audio_player_exist){
				toggleAudio();
			}else{
				//music button just toggles audio, nothing more
				//console.log(audioPlayerHolder.getMediaPlaying());
				if(audioPlayerHolder.getMediaPlaying()){
					audioPlayerHolder.pauseAudio();	
				}else{
					audioPlayerHolder.playAudio();	
				}
		...
		/*if(!isMobile)music_toggle.bind('mouseover',function(){
		...
		var audioOpened=false, audioIntroHappened=false, audio_player_exist=true;
		var audioForcePause=false;
		if($(ap_settings.playerHolder).hasClass('music_toggle')){
			audio_player_exist = false;
		}
	...
	function initTouch(){
		...
		thumbHolder.unbind('touchstart.ap touchmove.ap touchend.ap click.ap-touchclick').bind(
			'touchstart.ap',
			function(e){
				...
				startX = thumbHolder.position().left;
				startY = thumbHolder.position().top;
			...
			function(ev){
				...
				if(thumbsHorizontal){
					var value = startX - touchStartX + touchPos.pageX, w = getComponentSize('w');
					...
					if(value < w - allThumbsSize - scrollDifference){
						value=w - allThumbsSize - scrollDifference;	
						//toggleNextBtn('off');
					}else{
						//toggleNextBtn('on');
					}
					...
					var value=startY - touchStartY + touchPos.pageY, h = getComponentSize('h');
					...
					if(value < h - allThumbsSize - scrollDifference){
						value=h - allThumbsSize - scrollDifference;	
						//toggleNextBtn('off');
					}else{
						//toggleNextBtn('on');
					}
	...
	if(hasTouch && scrollType == 'buttons'){
		initTouch();
	}
	
	//************* fullscreen
	
	var useFullscreen=false, hasFullscreen=false;
	if(componentWrapper.find('.fullscreen_toggle').length){
		hasFullscreen=true;
	}
	if(hasFullscreen && checkFullScreenSupport()){
		useFullscreen=true;
		var fullscreen_toggle = componentWrapper.find('.fullscreen_toggle').css('cursor', 'pointer').bind('click', function(){
			if(!componentInited) return false;
			toggleFullscreen();
			return false;
		});
		...
		componentWrapper.find('.fullscreen_toggle').remove();
	}
	...
	function setFullscreenIcon(){
		...
		if(fullscreen_toggle)fullscreen_toggle.find('img').attr('src', url);
	}
	...
	var orig_liArr = [];//holds original li thumb holders
    ...
    var activeCategory;
	var currentCategory;
	var activeItem = settings.activeItem;
   ...
   //*** mobile pan
	...
	thumbHolder.find("div[class=category]").each(function(){
    ...
    function checkPlaylistProcess(){
		...
		if(processPlaylistArr.length){
			...
			//console.log('finish playlist process');	
			
			categoryDataArr = thumbHolder.find("div[class=category]");
			categoryLength = categoryDataArr.length;
			//console.log('categoryLength = ', categoryLength);
			
			getDeeplinkData();
			checkMenuThumbs();//from categories made from folders, make menu thumbnails 
			
		}
	}
	
	function checkMenuThumbs() {
		//console.log('checkMenuThumbs');
		var i = 0, type, cat, menu_counter = 0, len = 0;
		for(i; i< categoryLength;i++){
			cat = $(categoryDataArr[i]);
			if(cat.attr('data-type') != undefined && !isEmpty(cat.attr('data-type'))){
				type = cat.attr('data-type');
				if(type == 'folder' && cat.attr('data-menuThumbPath') != undefined && !isEmpty(cat.attr('data-menuThumbPath'))){
					len++;
				}
			}
		}
		...
		if(len > 0){
			...
			var thumb_src, remap, menu_thumb, thumb_ext, thumb_data, thumbArr=[];
			...
			for(i; i< categoryLength;i++){
				...
				if(cat.attr('data-type') != undefined && !isEmpty(cat.attr('data-type'))){
					...
					if(type == 'folder'){
						...
						thumbArr[i] = cat;//for later resize
						
						//copy first image from each category to be menu thumbnail to the menu folder, then later resize them
						
						//data-imagePath="media/images/category1/01.jpg", current path
						//data-menuThumbPath="media/menu/01.jpg", new path
						
						thumb_src = cat.attr('data-menuThumbPath');//current menu thumbpath is first image in each category
						remap = '../'+settings.menuThumbPath + stringCounter(i) + '.jpg';
						thumb_data = "src=" + thumb_src + "&dst=" + remap;
						//console.log(thumb_data);
						
						//set new menu thumb path
						cat.attr('data-menuThumbPath', remap);
						...
						$.ajax({
							...
							success: function() {
								...
								menu_counter++;
								if(menu_counter == len){
									//console.log('all menu thumbs copied');
									resizeMenuItems(thumbArr, menu_counter);
								}
							},
	...
	function resizeMenuItems(thumbArr, len){
		var i = 0, menu_counter = 0, thumb_src, remap, menu_thumb, thumb_ext, tw, th, menu_item, thumb_data;
		
		//get menu item width, height
		menu_item = $('<div/>').addClass('menuRowMasker').css('visibility','hidden').appendTo(componentWrapper);
		tw = menu_item.width(), th = menu_item.height();
		menu_item.remove();//clean
		menu_item = null;
		//console.log(tw, th);
		...
		for(i; i< categoryLength;i++){
			if(thumbArr[i]){
				...
				thumb_src = cat.attr('data-menuThumbPath');
				remap = '../'+settings.menuThumbPath;
				thumb_quality = 80;
				thumb_data = "src=" + thumb_src + "&dst=" + remap + "&tw=" + tw + "&th=" + th + "&quality=" + thumb_quality;
				//console.log(thumb_data);
				...
				$.ajax({
					...
					success: function(data) {
						...
						menu_counter++;
						if(menu_counter == len){
							//console.log('all menu thumbs resized');
							initMenu();
						}
					},
					error: function(XMLHttpRequest, textStatus, errorThrown) {
	...
	function folderParseHandler(data, response) {
		//console.log('folderParseHandler');
		
    	if(response.toLowerCase() === "success") {
			
			var i = 0, j, images = data[1], category = data[0], len = category.length, len2, div, ul, address, address2, menuThumbPath, li, imagePath, thumbPath, tw, th, thumb_counter, thumb_src, thumb_dst, thumb_ext, thumb_quality , thumb_data, thumb_path, remap;
			
			for(i; i < len; i++) {
				//console.log(category[i]);
				
				//<div data-type='html' class="category" data-address="fashion_one" data-menuThumbPath="media/menu/01.jpg" data-thumbWidth="80" data-thumbHeight="53" data-title="Category title number one" >
				
				//create categoy div holder for li items
				address=category[i], menuThumbPath = images[i][0]/*menu item from first image*/, tw = settings.thumbWidth, th = settings.thumbHeight;
				div = $("<div data-type='folder' data-address='"+address+"' data-menuThumbPath='"+menuThumbPath+"' data-thumbWidth='"+tw+"' data-thumbHeight='"+th+"' data-title='"+category[i]+"'/>" ).addClass('category');
				ul = $('<ul/>').appendTo(div);
				
				_currentInsert.after(div);
				_currentInsert=div;
				
				j = 0, len2 = images[i].length, thumb_counter = 0;
				for(j; j < len2; j++) {
					//console.log(images[i][j]);
					//create li items
					//<li data-address='image2' data-imagePath="media/images/category1/images/02.jpg" data-thumbPath="media/category1/thumbs/02.jpg" data-vimeo='14665315' />
					address2='image'+j, imagePath = images[i][j], thumbPath = '';
					li = $("<li data-address='"+address2+"' data-imagePath='"+imagePath+"'/>").appendTo(ul);
				
					//create thumbs
					thumb_src = images[i][j];
					//console.log(thumb_src);
					thumb_dst = thumb_src.substr(0, thumb_src.lastIndexOf('/')+1);//thumb folder path
					thumb_ext = thumb_src.substr(thumb_src.lastIndexOf('/')+1);//thumb name.extension
					remap = thumb_dst + 'thumbs/';
					thumb_path = remap + thumb_ext;
					thumb_quality = 80;
					thumb_data = "src=" + thumb_src + "&dst=" + remap + "&tw=" + tw + "&th=" + th + "&quality=" + thumb_quality;
					//console.log(thumb_data);
					
					li.attr('data-thumbPath', thumb_path);
					...
					$.ajax({
						...
						success: function() {
							//console.log('success', $(this));
							thumb_counter++;
							if(thumb_counter == len2){
								//console.log('all thumbs created');
								if(_currentInsertRemove)_currentInsertRemove.remove();//remove original li item that holds the data for creating playlist	
			...
			alert("Folder Parse Response: " + response);
		}
	}
	
	function processPlaylistItem(){
		//console.log('processPlaylistItem');
		var _item = $(processPlaylistArr[0]), data_type, data_path;
		
		if(_item.attr('data-type') != undefined && !isEmpty(_item.attr('data-type'))){
			data_type = _item.attr('data-type');
		}else{
			processPlaylistArr.shift();
			checkPlaylistProcess();
			return;
		}
		if(_item.attr('data-path') != undefined && !isEmpty(_item.attr('data-path'))){
		...
		if(data_type == 'html'){
			...
			_currentInsertRemove=_item;
			_currentInsert=_item;	
			_item.css('display','none');//hide data li
				
			$.getJSON('php/thumb_create.php' + "?path=" + '../' + data_path, folderParseHandler);	
			...
			$.ajax({
				...
				success: function(xml) {
					$(xml).find("div[class='category']").each(function(){
						obj = $(this);
						//console.log(obj);
						...
						cloner = document.createElement('div');
						cloner.innerHTML = str;
						div = cloner.firstChild;
				...
				error: function(s,x) { 
					alert('Parse xml error! Make sure you run this online or on local server!');
					alert('Playlist selection failed: ' + url);
					processPlaylistArr.shift();
					checkPlaylistProcess();
				}
			});
	...
	function initMenu(){
		...
		if(categoryLength > 1){
			...
			var data, categoryItem, i=0, menu_path;
			for(i; i < categoryLength; i++){
				  categoryItem=$(categoryDataArr[i]);
				  data = {};
				  menu_path = categoryItem.attr('data-menuThumbPath');
				  if(menu_path.indexOf('../')>-1){//remove '../' we used for php resize menu items
					  menu_path = menu_path.substring(3);
				  }
				  data.url=menu_path;
			...
			if(hasFullscreen && useFullscreen){
				galleryControls.removeClass('galleryControls').addClass('galleryControls_2icons');//no menu
			}else{
				galleryControls.removeClass('galleryControls').addClass('galleryControls_1icon');//no menu, no fullscreen
			}
	    }
    ...
    function initCategory(){
		...
		if(image_sizeToggleSrc) image_sizeToggleSrc.attr('src', ic_image_toggle_plus);
		...
		var i=0,li,a,thumb,_item, i_url, t_url, obj, target;
		...
		*/
		...
		current_category.find('ul li').each(function() {
	   		 _item = $(this);
			 orig_liArr.push(_item);
			 
			 obj = {};
			 i_url = _item.attr('data-imagePath');
			 if(i_url.indexOf('../')>-1){//remove '../' we used for php resize menu items
				i_url = i_url.substring(3);
			 }
			 obj.url = i_url;
			 ...
			 if(_item.attr('data-description') != undefined && !isEmpty(_item.attr('data-description'))){
			  ...
			  if(_item.attr('data-title') != undefined && !isEmpty(_item.attr('data-title'))){
			 ...
			 //load thumbs
			 t_url = _item.attr('data-thumbPath');
			 if(t_url.indexOf('../')>-1){//remove '../' we used for php resize menu items
				t_url = t_url.substring(3);
			 }
			 ...
			 if(!isMobile && scrollType == 'buttons'){
	...
	*/
	...
	function transitionFinishHandler() {
		if(!transitionOn){//when module transition finishes
			...
			if(!address_intro){
				address_intro=true;
				addressDelayFix(_externalChangeEvent);
			}else{
	...
	function addressDelayFix(e){
		address_intro=true;
		//console.log('addressDelayFix: ', e.value);
		if(e.value == "/"){
	...
	function setThumbScroll(){
		thumb_backward.bind(_downEvent, function(){
			if(_thumbScrollIntervalID) clearInterval(_thumbScrollIntervalID);
			_thumbScrollIntervalID = setInterval(function() { _scrollThumbsBack(); }, 100);
			return false;
		});
		thumb_backward.bind(_upEvent, function(){
			if(_thumbScrollIntervalID) clearInterval(_thumbScrollIntervalID);
			return false;
		});
		thumb_forward.bind(_downEvent, function(){
			if(_thumbScrollIntervalID) clearInterval(_thumbScrollIntervalID);
			_thumbScrollIntervalID = setInterval(function() { _scrollThumbsForward(); }, 100);
			return false;
		});
		thumb_forward.bind(_upEvent, function(){
			if(_thumbScrollIntervalID) clearInterval(_thumbScrollIntervalID);
			return false;
		});
	 }
	 ...
	 function checkThumbScroll(){//toggle visiblity of thumb scroll buttons
		 ...
		 if(scrollType == 'buttons'){
			...
			if(!scrollPane){
				scrollPane = thumbInnerContainer.jScrollPane().bind('jsp-initialised',function(event, isScrollable){
				...
				if(!isMobile){
					thumbInnerContainer.bind('mousewheel', function(event, delta, deltaX, deltaY){//init on both sides (not just horizontal) because layout can be switched!
						if(!componentInited || categoryTransitionOn) return false;
						if(thumbsHorizontal){
			 ...
			 if(thumbsHorizontal){
				//if(scrollPaneApi)_isScrollable = scrollPaneApi.getIsScrollableH();//recheck
				if(allThumbsSize <= getComponentSizeClear('w')){
					_isScrollable=false;
				}else{
					_isScrollable=true;
				}
			 }else{
				//if(scrollPaneApi)_isScrollable = scrollPaneApi.getIsScrollableV();
				if(allThumbsSize <= getComponentSizeClear('h')){
					isScrollable=false;
				}else{
					_isScrollable=true;
				}
			 }
			 //console.log('_isScrollable: ', _isScrollable);
			 if(!_isScrollable){
				centerThumbs(); 
			 }else{
				centerThumbs2(); 	
	 ...
	 function centerThumbs(){
		//console.log('centerThumbs');
		 //center thumbs if no scroll
		if(thumbsHorizontal){
			thumbWrapper.height(thumbHeight);
			$('.jspPane').css('left',Math.abs(getComponentSizeClear('w')/2-allThumbsSize/2)+'px');
		}else{
			thumbWrapper.width(thumbWidth);
			$('.jspPane').css('top',Math.abs(getComponentSizeClear('h')/2-allThumbsSize/2)+'px');
		}
	 }
	 
	  function centerThumbs2(){
		if(thumbsHorizontal){
			thumbWrapper.height(thumbHeight + scroll_size);
		}else{
			thumbWrapper.width(thumbWidth + scroll_size);
			scrollPaneApi.reinitialise();
		}
	 }
   ...
   function clickCategoryItem(e){
	...
	function cleanCategory(){
		...
		if(componentInited){
			...
			if(useInfo){
				...
				if(info_toggleSrc)info_toggleSrc.attr('src',ic_info_toggle);
			} 
	...
	function togglePreloader(dir){
		if(dir == 'on'){
			imagePreloader.stop().css('display', 'block').animate({'opacity': 1}, 500, "easeOutSine");
		}else{
	...
	function toggleMenuHandler(hidePreloader){//open,close menu
		...
		if(hidePreloader)togglePreloader('off');
	...
	function checkImageData(){
		...
		//check video
		if(useVideo){
			data = orig_liArr[_playlistManager.getCounter()];
			if(data.attr('data-vimeo') != undefined){
				videoType = 'vimeo';
				videoLink = data.attr('data-vimeo');
				toggleVideoBtn('on');
				videoExistInSlide=true;
		...
		if(typeof afterSlideChange !== 'undefined')afterSlideChange(_playlistManager.getCounter());
	}
	...
	function checkDescription() {
		...
		if(infoExist){
			//reuse p tag for description becuase of info_scrollPaneApi.reinitialise
			if(infoPTag){
	...
	function checkInfoSize() {
		...
		if(infoExist){
			//check width first
			var w = getComponentSizeClear('w'), h = getComponentSizeClear('h'), h1 = h - parseInt(info_holderMasker.css('top'),10);
			//check width				
			...
			if(h1 <= infoProposedH){
				...
				if(info_bg)info_bg.height(infoProposedH + 10);
			}	
			if(infoOpened)info_holderMasker.height(info_holder.height());//fix
			
			if(!info_scrollPaneApi){
				info_scrollPaneApi = info_holder.jScrollPane().data().jsp;
	...
	function toggleInfo() {
		...
		if(!infoOpened){
			...
			if(info_toggleSrc)info_toggleSrc.attr('src',ic_info_close);
			infoOpened=true;
		}else{
	...
	function mediaUnloadedAction(){
		
		if(typeof beforeSlideChange !== 'undefined')beforeSlideChange(_playlistManager.getCounter());
    ...
    function loadImage(){
	   ...
	   mediaUnloadedAction();
   	   ...
   	   var url = imageDataArr[_playlistManager.getCounter()].url;
	   ...
	   var description = "";
		...
		if(data.title && global_style==1){
			description =  "<div style='margin:0 auto;color:#fff;margin-top:10px;padding:10px;'><div class='title'>"+data.title+"</div></div>";
		}else if(data.description && global_style==2){
			description =  "<div style='margin:0 auto;color:#fff;margin-top:10px;padding:10px;'><div class='desc'>"+data.description+"</div></div>";
		}else if(global_style==3){
			if(data.title){
				description = "<div class='title'>"+data.title+"</div>";
			}
			if(data.description){
				description += "<div class='desc'>"+data.description+"</div>";
			}
			description =  "<div style='margin:0 auto;color:#fff;margin-top:10px;padding:10px;max-height:300px;overflow:hidden;'>"+description+"</div>";
		}
		
		var str ="<div class='panInner' style='text-align:center;vertical-align:middle;position:relative;opacity:0;overflow:hidden;'><img src='"+url+"' style='max-width:100%;max-height:100%;' >"+description+"</div>";
		

	   panHolder.find(".panInner").css("opacity","0");
	   var img = $(str).prependTo(panHolder).find("img");
	   img.load(function() {
			//hide preloader
			togglePreloader('off');
			nextImage = $(this).closest('.panInner');
		    imageLoaded=true;
   ...
   function setImage(){
	   ...
	   if(isMobile){
		   currentImage.bind("dragstart", function(e) { e.preventDefault(); return false; }).bind("selectstart", function(e) { e.preventDefault(); return false; }).touchSwipe(swipeHandler, true); 
	   }
	   ...
	   if(!componentInited){
			...
			if(useAudio && music_toggle)music_toggle.css({opacity: 0, display: 'block'}).stop().animate({ 'opacity': 1},  {duration: 1000, easing: 'easeOutSine'});
			
			if(useAudio && ap_settings.autoOpenAudioPlayer){
				toggleAudio();
			}
			
			if(typeof panGalleryReady !== 'undefined')panGalleryReady();
	   }
   }
   ...
   function checkImageSize(){//apply this only on beginning when image loads, otherwise apply checkImageSizeOnResize()
		 ...
		 if(imageStartScale == 'scaled'){
			 ...
			 if( global_imagesize && global_imagesize!=100){
				var oldimgwidth = 	currentImage.find("img").width();
				var newimgwidth =  oldimgwidth*global_imagesize/100;
				currentImage.find("img").width(newimgwidth);
				currentImage.find(" > div").width(newimgwidth-20);
				imageScaledHeight = currentImage.height();
			}else{
				var oldimgwidth = 	currentImage.find("img").width();
				currentImage.find(" > div").width(oldimgwidth-20);
			}
			 if(typeof global_imageheight !="undefined"  && global_imageheight!=100 ){
				currentImage.find("img").css("max-height",global_imageheight+"%");
				 var x = getComponentSize('w') / 2 - imageScaledWidth / 2;
				 var y = getComponentSize('h') / 2 - (imageScaledHeight*global_imageheight/100) / 2;
			 }else{
				 var x = getComponentSize('w') / 2 - imageScaledWidth / 2;
			...
			currentImage.css({
			 ...
			 if(panAbility){
				 imageEnlarged = true;
				 if(image_sizeToggleSrc)image_sizeToggleSrc.attr('src', ic_image_toggle_minus);//reverse
				 if(image_sizeToggle)image_sizeToggle.css('display','block').animate({opacity: 1}, {duration: 500,easing: 'easeOutSine'}); 
	...
	function hideNonPanElements(){
		...
		if(!isMobile){
			...
			if(useAudio && audioPlayerHolder){
				audioPlayerHolder.stop().animate({opacity: 0},{duration: time, easing: 'easeOutQuart', complete: function(){
					$(this).css('display','none');
				}});
				if(audio_player_exist)music_toggle.stop().animate({opacity: 0},{duration: time, easing: 'easeOutQuart', complete: function(){
					$(this).css('display','none');
				}}); 
			}
			if(useVideo && player_bigPlay)player_bigPlay.animate({opacity: 0},{duration: time, easing: 'easeOutQuart', complete: function(){
				$(this).css('display','none');
			}}); 
		}else{
			if(useInfo){
			...
			if(useAudio && audioPlayerHolder){
				audioPlayerHolder.css('display','none');
				music_toggle.css('display','none');
			}
			if(useVideo && player_bigPlay)player_bigPlay.css('display','none');
		}
		touchOn=false;
    ...
    function showNonPanElements(){
		...
		if(!isMobile){
			...
			if(useAudio && audioPlayerHolder){
				if(audioOpened)audioPlayerHolder.stop().css({opacity: 0, display:'block'}).animate({opacity: 1},{duration: time, easing: 'easeOutQuart'}); 
				music_toggle.css({opacity: 0, display:'block'}).animate({opacity: 1},{duration: time, easing: 'easeOutQuart'}); 
			}
			if(useVideo && player_bigPlay && videoExistInSlide)player_bigPlay.css({opacity: 0, display:'block'}).animate({opacity: 1},{duration: time, easing: 'easeOutQuart'});
		}else{
			time=0;
			if(useInfo){
				if(infoResizeQue){
			...
			if(useAudio && audioPlayerHolder){
				if(audioOpened)audioPlayerHolder.css({opacity: 1, display:'block'});
				music_toggle.css({opacity: 1, display:'block'});
			}
			if(useVideo && player_bigPlay && videoExistInSlide)player_bigPlay.css({opacity: 1, display:'block'});
		}
   ...
   function toggleImageSizeDone(){
		if(imageEnlarged){
			if(image_sizeToggleSrc)image_sizeToggleSrc.attr('src', ic_image_toggle_minus);
			if(isMobile){
				if(!mobileZoomInited){
					initMobileZoom();
				}else{
			...
			if(slideshowForcePause){
				slideshowForcePause=false;
				slideshowOn = true;
				slideshowToggleSrc.attr('src', ic_slideshow_pause);
			}
		}
		transitionOn = false;
		if(slideshowOn){
    ...
    function toggleSlideshow(){
		  ...
		  if(slideshowTimeoutID) clearTimeout(slideshowTimeoutID);
		  if(slideshowOn){
			  slideshowToggleSrc.attr('src', ic_slideshow_play);
			  slideshowOn=false;
		  }else{
			  slideshowToggleSrc.attr('src', ic_slideshow_pause);
			  slideshowOn=true;
   ...
   function forwardHandler(){
	...
	function makeThumbHolder(){
		...
		if(thumbsHorizontal){
			...
			if(scrollType == 'buttons'){
				...
				thumbWrapper.removeClass('thumbWrapper_v').addClass('thumbWrapper')
				.css({top:'auto', left:0+'px', height:thumbHeight+scroll_size+'px', width:100+'%'});
				thumbHolder.width(allThumbsSize).height(thumbHeight);
				thumbInnerContainer.removeClass().addClass('thumbInnerContainer').height(thumbHeight+scroll_size+'px');
			}
			...
			if(!dontAlignGalleryControls)galleryControls.css('marginLeft', -galleryControls.width()/2+'px');
			...
			if(scrollType == 'buttons'){
				...
				thumbWrapper.removeClass('thumbWrapper').addClass('thumbWrapper_v')
				.css({left:'auto', top:0+'px', width:thumbWidth+scroll_size+'px', height:100+'%'});
				thumbHolder.height(allThumbsSize).width(thumbWidth+'px');
				thumbInnerContainer.width(thumbWidth+scroll_size+'px').height(100+'%');
			}
			...
			if(!dontAlignGalleryControls)galleryControls.css('marginLeft', -(galleryControls.width()/2 + thumbWidth/2)+'px');
	...
	function resizeElements(){//resizes with no thumb change
		...
		if(!imageEnlarged){
			panHolder.css({height: getComponentSize('h')+'px', width: getComponentSize('w')+'px'});
			if(scrollType == 'buttons')thumbHolder.css({left: 0+'px', top: 0+'px'});
		}else{
			panHolder.css({height: getComponentSizeClear('h')+'px', width: getComponentSizeClear('w')+'px'});
		}
		if(scrollType == 'buttons'){
	...
	this.flipGallerySide = function(){
		if(!componentInited)return;
		thumbsHorizontal = !thumbsHorizontal;
		makeThumbHolder();
		doneResizing2();
	}
	...
	function doneResizing2(){
		...
		if(useVideo){
			player_bigPlay.css({
				left:getComponentSize('w')/2-player_bigPlay.width()/2,
				top:getComponentSize('h')/2-player_bigPlay.height()/2
			});	
		}
	}
	...
	function doneResizing(){
		...
		if(componentInited){
			...
			if(menuExist && allMenuRowMaskerArr.length){
				checkMenuSize();
				if(menuOpened){
					alignMenu();
				}
	...
	this.openCategory = function(value) {
		if(menuOpenToggle) return;
		if(imageEnlarged)return;
	...
	$.fn.panGallery.getSlideshowForcePause = function() {	
		return getSlideshowForcePause();
	}
	$.fn.panGallery.videoEnd = function() {	
		videoEnd();
	}
